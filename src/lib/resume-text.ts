...
/**
 * Pull numbers out of text. `token` is normalized so "$1.2M" and "$1,200,000" compare
 * equal; `raw` is what was written, for messages. Percent and multiplier ("3x") markers
 * are part of the token: "50%" is a different claim than "50 people". Dollar signs are
 * ignored.
 */
export function extractNumbers(text: string): { raw: string; token: string }[] {
  ...
  const numbers: { raw: string; token: string }[] = []
  for (const match of withoutMonths.matchAll(NUMBER_RE)) {
    ...
    numbers.push({ raw: match[0].trim(), token: `${Number(value.toPrecision(12))}${marker}` })
  }
  return numbers
}

export function extractNumberTokens(text: string): string[] {
  return extractNumbers(text).map((n) => n.token)
}
